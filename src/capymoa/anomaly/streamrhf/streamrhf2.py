...
def RHT_build(data, height, max_height, seed_array, node_id=0):
    ...
    node.left = RHT_build(left_data, height + 1, max_height, seed_array, node_id=2*node_id+1)
    node.right = RHT_build(right_data, height + 1, max_height, seed_array, node_id=2*node_id+2)
    return node
...
def insert(node, instance, max_height, seed_array):
    if not node.is_leaf():
        ...
        if node.attribute != new_attribute:
            # Rebuild from scratch at this node, node_id=0 since we create a new subtree
            return RHT_build(np.vstack((node.data, instance)), node.height, max_height, seed_array, node_id=0)
        ...
        if node.height == max_height:
            ...
            # Rebuild from scratch at this node
            return RHT_build(np.vstack((node.data, instance)), node.height, max_height, seed_array, node_id=0)
    return node
...
class RandomHistogramForest:
    ...
    def initialize_forest(self):
        ...
        num_nodes = 2 ** (self.max_height + 1) - 1
        ...
        for i in range(self.num_trees):
            tree = RHT_build(np.empty((0, self.number_of_features)), 0, self.max_height, self.seed_arrays[i], node_id=0)
    ...
    def update_forest(self, instance):
        ...
        if len(self.current_window) >= self.window_size:
            ...
            self.forest = []
            for i in range(self.num_trees):
                tree = RHT_build(np.array(self.reference_window), 0, self.max_height, self.seed_arrays[i], node_id=0)
                self.forest.append(tree)
...
max_height = 5
num_trees = 100
window_size = data_stream.shape[0] // 100 # 1% of the data stream
...
execution_time = 0.0  # Just as placeholder

print(f"AP = {ap_score:.4f}, Execution Time = {execution_time:.2f} seconds")
